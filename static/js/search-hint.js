...
function createSearchHintList (hints, lang) {
  ...
  if (hints == null || hints.length === 0) {
    ...
    for (var i = 0; i < hints.length; i++) {
      ulBox.innerHTML += '<li><a href="/' + lang + '/' + hints[i].subject + '/' + hints[i].topic + '/"><span class="search-hints-subject"><img src="' + hints[i].subject_icon_url + '" alt="' + hints[i].subject + '">' + hints[i].subject + '</span><span class="search-hints-topic" title="' + hints[i].topics + '"></span></a></li>'
      ulBox.querySelectorAll('li > a')[i].querySelector('span.search-hints-topic').textContent = hints[i].topics
    }
  }
}
